...
export default function Essence() {
  return <>
    <div id="essence" className="w-full h-auto min-h-screen bg-gray-100 px-4 py-8 md:px-42 md:py-20">
      <Reveal>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-[32px]">
        {cards.map((card, index) => (
          <Reveal key={card.name}
            ...
            tabIndex={0}
            className={` fade-up delay-${index % 4}`}>
            <div key={card.name} className={`bg-white border ${borderMap[card.color] ?? "border-black"} p-8 rounded-[32px] flex flex-col gap-[12px] h-full md:gap-[24px] items-center justify-start shadow-md transition-all duration-300 ease-in-out hover:shadow-lg hover:border-b-8 cursor-pointer group`}>
              <div className={`w-[64px] h-[64px] flex items-center justify-center ${colorBgMap[card.color] ?? "bg-gray-100"} rounded-full transition-transform duration-300 ease-in-out group-hover:scale-125`}>
                {card.icon}
              </div>
      ...
      <Reveal>
        <div className="relative mt-12 text-center space-y-6  md:max-w-[70%] mx-auto px-4 md:px-20 py-8 rounded-4xl group cursor-pointer">
          {/* Gradiente base */}
          <div className="absolute inset-0 bg-gradient-to-r from-[#084A96] to-[#187EF1] transition-opacity duration-500 ease-in-out rounded-4xl"></div>
          {/* Gradiente hover */}
          <div className="absolute inset-0 bg-gradient-to-r bg-[#187EF1] opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out rounded-4xl"></div>
          {/* Conteúdo */}
